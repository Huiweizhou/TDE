...
class EvolveWeights(object):
    def __init__(self, row, col):
        super(EvolveWeights, self).__init__()
        self.update = EvolveGate(row, col, tf.sigmoid)

        self.reset = EvolveGate(row, col, tf.sigmoid)

        self.htilda = EvolveGate(row, col, tf.tanh)

    def __call__(self, inputs, vecs):
        # inputs: (300, 256)
        # vecs: (batch, 300)
        ...
        indices = []
        for _ in range(256):  # batch_size
            indices.append(_ * 10)
        vecs = tf.gather(vecs, indices)
        z_topk = tf.transpose(vecs)
        update = self.update(z_topk, inputs)
        reset = self.reset(z_topk, inputs)

        h_cap = reset * inputs
        h_cap = self.htilda(z_topk, h_cap)

        new_Q = (1 - update) * inputs + update * h_cap

        return new_Q


class EvolveGate(object):
    def __init__(self, row, col, act):
        super(EvolveGate, self).__init__()
        self.activation = act
        self.W = glorot([row, row])
        self.U = glorot([row, row])
        self.bias = zeros([row, col])

    def reset_param(self, t):
        # Initialize based on the number of columns
        stdv = 1. / math.sqrt(t.shape[1])
        t = tf.random.uniform(shape=t.shape, minval=-stdv, maxval=stdv)
...
class RnnEvolve(object):
    def __init__(self,name, row, col):
        super(RnnEvolve, self).__init__()

        self.W = glorot([row, row])
        self.U = glorot([row, row])
        self.bias = tf.Variable(tf.random.uniform(shape=[row, col]))
    ...
    def __call__(self, x, y):
        out = tf.sigmoid(tf.matmul(self.W, x) + \
                         tf.matmul(self.U, x) + \
                         self.bias)
        return out
...
class GCNAggregator(Layer):
    ...
    def __init__(self, input_dim, output_dim, neigh_input_dim=None,
        ...
        self.input_dim = input_dim
        # print(self.vars['weights'])
        self.output_dim = output_dim
        self.evolve_weights = EvolveWeights(neigh_input_dim, output_dim)  # (300, 256)
...
class MaxPoolingAggregator(Layer):
    ...
    def __init__(self, input_dim, output_dim, model_size="small", neigh_input_dim=None,
        ...
        # self.mlp_layers = []
        self.mlp_layers = Dense(input_dim=neigh_input_dim,
    ...
    def _call(self, inputs):
        ...
        h_reshaped = self.mlp_layers(tf.reshape(neigh_vecs, (batch_size * num_neighbors, self.neigh_input_dim)))
