...
class SupervisedGraphsage(models.SampleAndAggregate):
    ...
    def __init__(self, pairs, labels, num_window,
        ...
        self.tmp_ht, self.l_t, _, self.losses = tf.scan(self.forward, self.range,
                                                        initializer=[h_0, tmp_h_0, pre_label,
                                                                     losses],
                                                        parallel_iterations=1,
                                                        name='h_t_transposed', swap_memory=True)
        self.loss = tf.reduce_mean(self.losses)
        tf.compat.v1.summary.scalar('loss', self.loss)
        tf.compat.v1.summary.histogram('h_t', self.l_t[-1, :, :])
        ...
        self.pre_preds=self.l_t
        self.preds = tf.nn.sigmoid(self.node_pred(self.l_t[-1, :, :]))
